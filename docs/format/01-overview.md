...
The report definition itself is small — tens of kilobytes inflated, a median of about 20 KB across this project's
fixture corpus — regardless of the overall file size. Large files are large because of embedded images and cached data,
not because of the definition.
...
flowchart LR
    ...
    E --> F["Split into records<br/>TSLV framing"]
    F --> G["Build record tree<br/>stack-XOR mask"]
    G --> H(["Build<br/>semantic model"])
```
...
5. **Split into records.** The logical bytes are a flat sequence of length-delimited records (the _TSLV_ framing: type,
   schema, length, value).
...
6. **Build the record tree.** Each record's content is itself a nested sequence of records. A masking scheme (an XOR
   ...
   the _lossless record layer_. See [The record tree](04-record-tree.md).

7. **Build the semantic model.** The record tree is walked and interpreted into a typed report model: database,
   parameters, formulas, groups, sections, and report objects. See [The semantic model](../reader/01-semantic-model.md).

Stages 1–6 are fully reversible and lossless: every record is preserved, including types the library does not yet
understand. Stage 7 is built on top — it grows over time without ever risking the round-trip.
...
format, not a decode choice; see [Endianness](07-endianness.md).
...
The following documents each drill into one stage; the [usage guide](../reader/03-cli.md) covers the full CLI.
