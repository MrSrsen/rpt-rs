...
[Stream decoding](03-stream-decoding.md)), except that its nested records use the relaxed **QE** framing dialect rather
than the `Contents` one (varied subtypes — see [The record tree](04-record-tree.md)); `rpt dump --stream qe` is how you
look at its bytes. `SavedRecordsStream` and `MemoValuesStream` are **batches** (below).
...
`report.saved_data: Option<SavedData>` (record count, columns, row-major cell values). See [Usage](11-usage.md).
...
- **Batch class.** Two stored layouts decode. The **memo-heap** class keeps variable-length values in an external
  ...
  does not yield a valid decryption IV still emits `"saved_data": null`.

---

← [The semantic model](05-semantic-model.md) · [Index](README.md) · **Next:** [Block catalog](07-block-catalog.md) →
