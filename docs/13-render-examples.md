...
resolution — see [Rendering](12-rendering.md); for the CLI, see [Usage](11-usage.md).
...
`RenderSource` (saved data, a materialized `RowSource`, a pre-built `Dataset`) always succeeds, because fetching the rows
happened before the call. Because `RenderOptions` derives `Default`, set only the fields you need and spread the rest;
...
`RenderOptions::as_of` fixes the instant the date/time specials (`CurrentDate`/`Today`, `CurrentDateTime`, `CurrentTime`)
resolve to. Left `None`, it captures the system clock once at render start, so the whole render is internally consistent
but differs run to run — set it explicitly for frozen baselines and oracle diffs. On `wasm32` there is no wall clock, so
the default falls back to the Unix epoch and a host that needs a real date must supply this.
...
`render_ir_json(report)` is a fifth output: the normalized Page-IR JSON, one string per page — not a backend but the
surface the render-parity tooling diffs against a reference, and the handiest thing to snapshot in a test.
...
See [Usage](11-usage.md#database-configuration---db) for the full CLI contract.
...
`rpt-render` has exactly one feature, `cosmic`, on by default; it pulls the native system-font scan, so disable it for a
WASM target. (The `db-postgres` / `db-sqlite` features belong to the **`rpt-render-cli`** binary, not the library — the
facade never links a driver, so there is nothing to turn off here.)
...
`render_backend`, and the facade's `to_*` / `export_svg_pages` methods — returns its output directly, not a `Result`.
Only two things in this crate are fallible, and neither is the render:

- `ReportDocument::load` returns `rpt::Result` (a decode error).
- `export_pdf_to_disk` / `export_html_to_disk` return `rpt::Result` (writing the file). They return `rpt::Result` rather
  than `std::io::Result` so the error **names the path** — an embedding caller gets the same "which file?" answer the
  CLI does, instead of a bare `No such file or directory`.
...
`Db` / `Io` / `Output` variants — because resolving a connection URL, coercing `-p` values, and writing multi-page output
are all things the binary owns. It is a property of the CLI, not of the library. A `Db` failure additionally carries a
`hint()`: the failing statement, and for a missing table or column a pointer at `rpt sql <file>`.

---

← [Rendering](12-rendering.md) · [Index](README.md) · **Next:** [The formula engine](formula-engine/README.md) →
