...
- **`Rnd`** — Crystal's VBA-derived seeded PRNG needs persistent mutable seed state across calls (an evaluation-context
  concern, not a pure function), and the native negative-seed reseed rule is unknown. Deferred until the eval
...
- **`Roman(n, form)` simplified forms (1–4)** — only the classic form (0 / omitted) is implemented; the graduated
  simplified forms' exact per-level rules are unknown.

Control forms `IIf`, `Switch`, and `Choose` are handled in the evaluator itself (they are lazy — only the selected
branch runs), not in the builtins module.
...
Built on `rpt_format_value::civil` (`Date`/`Time` day-number and second arithmetic — no external date dependency).
Crystal follows VBA semantics; the notable rules are called out below.
...
Crystal's relative-date-range constants — the values a record-selection formula compares a date field against
(`{orders.date} in LastFullMonth`). Each is **0-ary** (a constant, not a call) and evaluates to an inclusive
`Value::Range` of dates computed against a reference "today": the context's `CurrentDate` / `Today` special, i.e. the
print date, so a report's ranges are stable across a run.

| Family              | Constants                                                                                                                    |
|---------------------|------------------------------------------------------------------------------------------------------------------------------|
| To-date             | `WeekToDateFromSun`, `MonthToDate`, `YearToDate`, `LastYearMTD`, `LastYearYTD`                                               |
| Recent / complete   | `Last7Days`, `Last4WeeksToSun`, `LastFullWeek`, `LastFullMonth`                                                              |
| Open-ended          | `AllDatesToToday`, `AllDatesToYesterday`, `AllDatesFromToday`, `AllDatesFromTomorrow`                                        |
| Aging buckets       | `Aged0To30Days`, `Aged31To60Days`, `Aged61To90Days`, `Over90Days`                                                            |
| Forward buckets     | `Next30Days`, `Next31To60Days`, `Next61To90Days`, `Next91To365Days`                                                          |
| Calendar partitions | `Calendar1stQtr` … `Calendar4thQtr`, `Calendar1stHalf`, `Calendar2ndHalf`                                                    |

Weeks start on Sunday (the engine's fixed convention for these constants — unlike `DayOfWeek`, they take no
`firstDayOfWeek` argument). Every bound is inclusive; the aging and forward buckets are day-count offsets from `today`,
and the calendar partitions are fixed month boundaries of the current year. The open-ended ranges are closed at the
representable date extremes rather than carrying an "unbounded" marker, so a plain `lo ≤ x ≤ hi` test is always valid.
...
`IRR`/`Rate` solve by Newton iteration and report `BadArg` if they do not converge. `NPV`/`IRR` require an array of cash
flows.
...
the data pipeline and report `Unsupported`. Crystal has no array-literal `Median`/`Mode` (summary-only — see the
deferred list at the top).
...
If the function's static return type isn't already in `types_table.rs`, add it there so type deduction agrees with
evaluation.
