...
workspace (the data/layout pipeline) depend on `crystal-formula` directly.
...
- **It has no dependency on the `rpt` decoder** — only on `rpt-format-value` (a dependency-free leaf, because a `Value`
  carries `Date`/`Time`). So it is reusable without pulling in the whole binary decoder: the upcoming Crystal LSP
  server, a WASM formula sandbox, and a standalone validator/playground can all depend on just `crystal-formula`.
...
← [Render examples](../13-render-examples.md) · [Index](../README.md) ·
**Next:** [Architecture & VM](01-architecture.md) →
