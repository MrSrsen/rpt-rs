...
/// first record with a scalar probe, or on failure the raw ciphertext head plus the deterministic
/// directory and derived-IV metadata for the undecodable batch.
pub(super) fn dump_saved(files: &[String], opts: &DumpOpts) -> Result<(), CliError> {
    for file in files {
        ...
        for (i, b) in insp.batches.iter().enumerate() {
            ...
            if b.decrypts_zlib {
                ...
                // Undecodable batch: report the deterministic directory + derived-IV metadata so the
                // mismatch is visible, without searching for the IV words.
                let _ = writeln!(
                    out,
                    "      undecodable — dir(count={}, item_size={}) · IV(batch_size={}, item_count={}, item_size={}, seq={})",
                    b.dir_count,
                    b.dir_item_size,
                    b.iv_batch_size,
                    b.iv_item_count,
