...
/// A chart object's decoded data binding — which report fields feed its values and categories. Chart
/// internals aren't otherwise surfaced anywhere else, so this is the only human-facing view for
/// verifying a chart's bindings during report authoring.
#[derive(Serialize)]
#[serde(rename_all = "camelCase")]
...
struct StreamHeadJson {
    ...
    bytes: usize,
    encrypted: bool,
    /// The header's `useFixed` flag. The decode path never branches on it (every encrypted stream
    /// seen so far uses the universal fixed key), so it is surfaced purely so a stream that does
    /// NOT use it is detectable rather than silently mis-decoded.
    fixed_key: bool,
    version: u16,
}
...
pub(crate) fn inspect(file: &str, json: bool) -> Result<(), CliError> {
        ...
        print_json(&InspectReport {
            ...
            streams: rpt
                ...
                .map(|(id, s)| StreamHeadJson {
                    ...
                    fixed_key: s.header().is_some_and(|h| h.use_fixed_key),
    ...
    println!(
        ...
        report.version, record_count, distinct_record_types
    );
    ...
    for (id, stream) in rpt.streams() {
        ...
        // A stream that failed to decode still lists (the bytes round-trip), so say why — otherwise
        // it is indistinguishable from an empty one.
        let err = match stream.decode_error() {
            Some(e) => format!("\n      !! {e}"),
            None => String::new(),
        };
        println!(
