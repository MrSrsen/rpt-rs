//! Conversions the PDF writer works in: twip→point, text-anchor math, and the Bézier constant.
...
/// renders as a thin visible line rather than vanishing.
...
/// metrics (centre/right placement only; the run is shaped by the real font when actually drawn).
