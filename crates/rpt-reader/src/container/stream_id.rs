...
pub enum StreamId {
    ...
    /// Any stream we do not classify — carries the full OLE path verbatim.
    Other(String),
}
...
impl StreamId {
    ...
    /// True for a query-engine session stream — the main report's `QESession` or a subreport's
    /// own one under its `Subdocument N` storage.
    pub(crate) fn is_qe_session(&self) -> bool {
        match self {
            StreamId::QESession => true,
            StreamId::Other(path) => path.ends_with("/QESession"),
            _ => false,
        }
    }

    /// True for the TSLV streams the codec layer decodes (`Contents` and `ReportParametersStream`).
