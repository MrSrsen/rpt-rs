//! The CFB/OLE2 compound-file container.
//!
...
/// One stream of a top-level `Embedding N` storage: its name with the OLE control-char prefix
/// stripped, and its bytes.
pub(crate) struct EmbeddingStream<'a> {
    pub name: String,
    pub bytes: &'a [u8],
}
...
impl Container {
    ...
    /// The streams of every top-level `Embedding N` storage, in directory order. Only top-level
    /// embeddings are enumerated: a nested `Subdocument N/Embedding …` has three path components
    /// and belongs to that subreport.
    pub(crate) fn embedding_streams(&self) -> Vec<EmbeddingStream<'_>> {
        self.streams
            .iter()
            .filter_map(|s| {
                ...
                storage.starts_with("Embedding ").then(|| EmbeddingStream {
                    // The stream name carries a `\x01`/`\x02` (OLE control) prefix.
                    name: stream.chars().filter(|c| !c.is_control()).collect(),
                    bytes: &s.bytes,
                })
            })
            .collect()
...
pub struct SummaryInformation {
    ...
    /// When the report was created (`PID_CREATE_DTM`), as a raw Windows `FILETIME`.
    pub created: Option<u64>,
    /// When the report was last saved (`PID_LASTSAVE_DTM`), as a raw Windows `FILETIME`.
    ///
    /// This is what the engine's `ModificationDate` / `ModificationTime` special fields report.
    pub last_saved: Option<u64>,
    /// When the report was last printed (`PID_LASTPRINTED`), as a raw Windows `FILETIME`.
...
// The layout of an MS-OLEPS property-set stream: a fixed header naming the property sets, then one
// section per set — its size and property count, a table of `(property id, value offset)` entries,
// and the values those entries point at.

/// The header ahead of the first section: byte order(2), version(2), system id(4), class id(16),
/// the property-set count(4), and the one `[FMTID(16) + section offset(4)]` entry a
/// `SummaryInformation` stream carries.
const PROPERTY_SET_HEADER_LEN: usize = 48;
/// The byte-order mark the header opens with — the stream is little-endian.
const BYTE_ORDER_MARK: u16 = 0xFFFE;
/// Where the header states how many property sets follow.
const SET_COUNT_OFF: usize = 24;
/// Where the first set's section offset sits: past the set table's leading FMTID.
const FIRST_SECTION_OFF: usize = 28 + 16;

/// Where a section states its property count, and where its entry table begins — both from the
/// start of the section, which is what every offset inside it is relative to.
const SECTION_COUNT_OFF: usize = 4;
const SECTION_TABLE_OFF: usize = 8;
/// One entry of that table: the property id, then its value's section-relative offset.
const SECTION_ENTRY_LEN: usize = 8;
const ENTRY_VALUE_OFF: usize = 4;
/// A value opens with its `vt` type tag and its length, and the body follows both.
const VALUE_LENGTH_OFF: usize = 4;
const VALUE_BODY_OFF: usize = 8;
...
impl SummaryInformation {
    ...
    fn parse(data: &[u8]) -> Option<SummaryInformation> {
        if data.len() < PROPERTY_SET_HEADER_LEN || u16_le(data, 0)? != BYTE_ORDER_MARK {
            return None;
        }
        if u32_le(data, SET_COUNT_OFF)? < 1 {
            return None;
        }
        let first_set_off = u32_le(data, FIRST_SECTION_OFF)? as usize;
        let sect = data.get(first_set_off..)?;

        let count = u32_le(sect, SECTION_COUNT_OFF)? as usize;
        let mut info = SummaryInformation::default();
        for i in 0..count {
            let entry = SECTION_TABLE_OFF + i * SECTION_ENTRY_LEN;
            let pid = u32_le(sect, entry)?;
            let voff = u32_le(sect, entry + ENTRY_VALUE_OFF)? as usize;
            // The thumbnail is a (non-string) clipboard blob; only its presence matters.
            if pid == PID_THUMBNAIL {
                info.has_thumbnail = true;
                continue;
            }
            // The timestamps are VT_FILETIME, not strings, so they are read before the string
            // path — which would otherwise reject them and drop the property silently.
            match pid {
                PID_CREATE_DTM => {
                    info.created = read_filetime_property(sect, voff);
                    continue;
                }
                PID_LASTSAVE_DTM => {
                    info.last_saved = read_filetime_property(sect, voff);
                    continue;
                }
                PID_LASTPRINTED => {
                    info.last_printed = read_filetime_property(sect, voff);
                    continue;
                }
                _ => {}
            }
...
pub(crate) fn scrub_identity_properties(data: &[u8]) -> Option<ScrubbedPropertySet> {
    if data.len() < PROPERTY_SET_HEADER_LEN
        || u16_le(data, 0)? != BYTE_ORDER_MARK
        || u32_le(data, SET_COUNT_OFF)? < 1
    {
        return None;
    }
    let sect_off = u32_le(data, FIRST_SECTION_OFF)? as usize;
    let count = u32_le(data.get(sect_off..)?, SECTION_COUNT_OFF)? as usize;

    let mut out = data.to_vec();
    let mut removed = Vec::new();
    for i in 0..count {
        let entry = sect_off + SECTION_TABLE_OFF + i * SECTION_ENTRY_LEN;
        ...
        let voff = sect_off + u32_le(data, entry + ENTRY_VALUE_OFF)? as usize;
        ...
        // `len` counts characters for VT_LPSTR and UTF-16 code units for VT_LPWSTR, so scale it to
        // bytes before blanking.
        let vt = u32_le(data, voff)?;
        let units = u32_le(data, voff + VALUE_LENGTH_OFF)? as usize;
        let bytes = match vt {
            VT_LPSTR => units,
            VT_LPWSTR => units * 2,
            _ => continue,
        };
        let body = voff + VALUE_BODY_OFF;
...
/// Read a `VT_FILETIME` property as its raw `FILETIME` — 100-nanosecond intervals since
/// 1601-01-01 UTC, little-endian.
///
/// Kept in the stored form rather than converted to a calendar date: the conversion needs a
/// timezone policy the file does not carry, and that is a consumer's decision.
///
/// A zero `FILETIME` reads as absent. The engine writes one for a report that has never been
/// printed, and reporting the 1601 epoch as a real date would be worse than reporting nothing.
fn read_filetime_property(sect: &[u8], off: usize) -> Option<u64> {
    if u32_le(sect, off)? != VT_FILETIME {
        return None;
    }
    // A `VT_FILETIME` carries no length word: the two halves of the timestamp follow the type tag.
    let lo = u32_le(sect, off + 4)? as u64;
    let hi = u32_le(sect, off + 8)? as u64;
    let ft = (hi << 32) | lo;
    (ft != 0).then_some(ft)
}
...
fn read_string_property(sect: &[u8], off: usize) -> Option<String> {
    ...
    let len = u32_le(sect, off + VALUE_LENGTH_OFF)? as usize;
    let body = sect.get(off + VALUE_BODY_OFF..)?;
    match vt {
        VT_LPSTR => {
