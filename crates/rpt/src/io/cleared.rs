//! The cleared-for-editing allow-list — which records the writer will edit without being forced.
//!
//! The write path validates only *mechanical* bounds: the record exists, the region is inside its
//! leaf, recomputed length prefixes fit. Those checks cannot catch the failure that matters. A
//! record whose leaf carries its own offset table, an element count, or a checksum can be
...
use crate::error::{Error, ProjectErrorKind};

/// Whether the writer enforces the cleared-for-editing allow-list.
///
/// The default is [`Checked`](EditPolicy::Checked): an edit to a record type whose internals are
/// not understood is refused rather than silently producing a damaged file.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default)]
pub enum EditPolicy {
    /// Refuse an edit to a record type that is not cleared for safe editing. The default.
    #[default]
    Checked,
    /// Edit regardless of clearance, for callers whose purpose *is* to write a record the reader
...
///
/// Clearance is per record *type* today because no cleared record has needed a finer rule. A record
/// whose leaf is only partly safe (a fixed-width scalar field beside an offset table, say) would add
/// a region bound to this entry rather than being cleared wholesale.
const CLEARED: &[Cleared] = &[Cleared {
    rtype: 0x0142,
    name: "SubreportReimportInfo",
    why: "its leaf is a big-endian length-prefixed path followed by a fixed 17-byte trailer, so it \
...
/// # Errors
///
/// [`Error::Project`] with [`ProjectErrorKind::UnclearedRecordEdit`], naming the record type and
...
pub(crate) fn check(rtype: u16, policy: EditPolicy) -> Result<(), Error> {
    ...
    let cleared = CLEARED
        ...
        .collect::<String>();
    Err(Error::Project {
        kind: ProjectErrorKind::UnclearedRecordEdit,
        detail: format!(
            "record type {rtype:#06x} is not cleared for editing. Its leaf may carry an internal \
...
mod tests {
    ...
    fn an_uncleared_record_is_refused_with_the_reason_and_the_escape_hatch() {
        ...
        let Error::Project { kind, detail } = &err else {
            panic!("expected a Project error, got {err:?}");
        };
        assert_eq!(*kind, ProjectErrorKind::UnclearedRecordEdit);
