//! The type-strict record DOM — a tree of domain DTOs, one struct per record type.
//!
...
//!
//! This is a projection over the raw substrate, built on demand from a [`RecordStream`]
//! (see [`crate::Rpt::record_dom`]); it is not part of the format-neutral semantic model.
//!
...
#[cfg_attr(feature = "serde", derive(serde::Serialize, serde::Deserialize))]
pub enum Node {
    /// SDK field definition. Boxed to keep the enum small: `FieldDef` is far larger than the
    /// `Unknown` variant, and most nodes in a decoded tree are `Unknown`.
...
impl RecordTypeCount {
    /// The record type's symbolic name, if this type has been identified; `None` for an unnamed
    /// type. Derived from [`tag`](Self::tag) via [`RecordTag::name`].
    pub fn name(&self) -> Option<&'static str> {
        RecordTag(self.tag).name()
    }
}
