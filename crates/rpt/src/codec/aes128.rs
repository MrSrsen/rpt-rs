//! L0.5 — the shared AES-128 core.
//!
//! Both payload ciphers ([`super::crypto`] for `Contents`, [`super::qe_crypto`] for `QESession`) run
//! the **same** block encryption and CFB-128 loop over the standard AES S-box and T-tables; they
//! differ only in where their 44 round-key words come from (a pre-expanded fixed schedule vs. a
//! standard key expansion). This module holds the shared S-box, T-table derivation, block function
//! and CFB loop; each cipher module supplies only its own `[u32; 44]` round keys.
//!
//! The block function uses the engine's transposed-state convention (the round mixes input bytes
//! `in[3], in[6], in[9], in[12]` into output word 0), which is a property of the round function, not
//! the key — so both ciphers share it.

/// Standard AES S-box.
#[rustfmt::skip]
const SBOX: [u8; 256] = [
    ,
];

/// The four AES T-tables, derived once from the S-box (`Te0[x] = [2·s, s, s, 3·s]` big-endian).
struct Tables {
    te: [[u32; 256]; 4],
}

fn xtime(a: u8) -> u8 {
    (a << 1) ^ if a & 0x80 != 0 { 0x1b } else { 0x00 }
}

/// Apply the S-box to each byte of a word (used by the standard key schedule).
pub(crate) fn sub_word(w: u32) -> u32 {
    (u32::from(SBOX[(w >> 24) as usize]) << 24)
        | (u32::from(SBOX[(w >> 16) as usize & 0xff]) << 16)
        | (u32::from(SBOX[(w >> 8) as usize & 0xff]) << 8)
        | u32::from(SBOX[w as usize & 0xff])
}

fn tables() -> &'static Tables {
    use std::sync::OnceLock;
    static T: OnceLock<Tables> = OnceLock::new();
    T.get_or_init(|| {
        let mut te0 = [0u32; 256];
        for (x, slot) in te0.iter_mut().enumerate() {
            let s = SBOX[x];
            let s2 = xtime(s);
            let s3 = s2 ^ s;
            *slot =
                (u32::from(s2) << 24) | (u32::from(s) << 16) | (u32::from(s) << 8) | u32::from(s3);
        }
        let mut te = [[0u32; 256]; 4];
        te[0] = te0;
        for x in 0..256 {
            te[1][x] = te0[x].rotate_right(8);
            te[2][x] = te0[x].rotate_right(16);
            te[3][x] = te0[x].rotate_right(24);
        }
        Tables { te }
    })
}

/// AES-128 block **encryption** (transposed-state convention) with the caller's 44 round-key words,
/// used by CFB to make the keystream.
pub(crate) fn encrypt_block(input: &[u8; 16], rk: &[u32; 44]) -> [u8; 16] {
    let t = tables();
    let (te0, te1, te2, te3) = (&t.te[0], &t.te[1], &t.te[2], &t.te[3]);
    let mut a = u32::from_le_bytes([input[0], input[1], input[2], input[3]]) ^ rk[0];
    let mut b = u32::from_le_bytes([input[4], input[5], input[6], input[7]]) ^ rk[1];
    let mut c = u32::from_le_bytes([input[8], input[9], input[10], input[11]]) ^ rk[2];
    let mut d = u32::from_le_bytes([input[12], input[13], input[14], input[15]]) ^ rk[3];

    for r in 1..10 {
        let k = 4 * r;
        let na = te0[(a >> 24) as usize]
            ^ te1[(b >> 16) as usize & 0xff]
            ^ te2[(c >> 8) as usize & 0xff]
            ^ te3[d as usize & 0xff]
            ^ rk[k];
        let nb = te0[(b >> 24) as usize]
            ^ te1[(c >> 16) as usize & 0xff]
            ^ te2[(d >> 8) as usize & 0xff]
            ^ te3[a as usize & 0xff]
            ^ rk[k + 1];
        let nc = te0[(c >> 24) as usize]
            ^ te1[(d >> 16) as usize & 0xff]
            ^ te2[(a >> 8) as usize & 0xff]
            ^ te3[b as usize & 0xff]
            ^ rk[k + 2];
        let nd = te0[(d >> 24) as usize]
            ^ te1[(a >> 16) as usize & 0xff]
            ^ te2[(b >> 8) as usize & 0xff]
            ^ te3[c as usize & 0xff]
            ^ rk[k + 3];
        a = na;
        b = nb;
        c = nc;
        d = nd;
    }

    // Final round: SubBytes + ShiftRows (no MixColumns), then AddRoundKey.
    let fin = |x0: u32, x1: u32, x2: u32, x3: u32, k: u32| -> u32 {
        ((u32::from(SBOX[(x0 >> 24) as usize]) << 24)
            | (u32::from(SBOX[(x1 >> 16) as usize & 0xff]) << 16)
            | (u32::from(SBOX[(x2 >> 8) as usize & 0xff]) << 8)
            | u32::from(SBOX[x3 as usize & 0xff]))
            ^ k
    };
    let oa = fin(a, b, c, d, rk[40]);
    let ob = fin(b, c, d, a, rk[41]);
    let oc = fin(c, d, a, b, rk[42]);
    let od = fin(d, a, b, c, rk[43]);

    let mut out = [0u8; 16];
    out[0..4].copy_from_slice(&oa.to_le_bytes());
    out[4..8].copy_from_slice(&ob.to_le_bytes());
    out[8..12].copy_from_slice(&oc.to_le_bytes());
    out[12..16].copy_from_slice(&od.to_le_bytes());
    out
}

/// Decrypt `ciphertext` in CFB-128 mode with `rk` (keystream block = `E(prev_ciphertext)`, first
/// block = `E(iv)`). Returns the plaintext (same length).
pub(crate) fn cfb_decrypt(iv: &[u8; 16], ciphertext: &[u8], rk: &[u32; 44]) -> Vec<u8> {
    let mut out = Vec::with_capacity(ciphertext.len());
    let mut feedback = *iv;
    for chunk in ciphertext.chunks(16) {
        let ks = encrypt_block(&feedback, rk);
        for (i, &c) in chunk.iter().enumerate() {
            out.push(c ^ ks[i]);
        }
        if chunk.len() == 16 {
            feedback.copy_from_slice(chunk);
        }
        // a partial final chunk ends the stream; no further feedback needed.
    }
    out
}

/// Encrypt `plaintext` in CFB-128 mode with `rk` — the inverse of [`cfb_decrypt`] (feedback is the
/// ciphertext just produced). Returns the ciphertext (same length).
pub(crate) fn cfb_encrypt(iv: &[u8; 16], plaintext: &[u8], rk: &[u32; 44]) -> Vec<u8> {
    let mut out = Vec::with_capacity(plaintext.len());
    let mut feedback = *iv;
    for chunk in plaintext.chunks(16) {
        let ks = encrypt_block(&feedback, rk);
        let start = out.len();
        for (i, &p) in chunk.iter().enumerate() {
            out.push(p ^ ks[i]);
        }
        // Feedback for the next block is the ciphertext just produced; a partial final chunk
        // ends the stream.
        if chunk.len() == 16 {
            feedback.copy_from_slice(&out[start..start + 16]);
        }
    }
    out
}
