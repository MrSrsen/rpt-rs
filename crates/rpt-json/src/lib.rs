...
//! [`rpt::model::Report`], including defaults and the whole subreport tree. See [`export_json`].
//!
...
//!
//! Reader-side: it links the `rpt` decoder, so it is *not* WASM-safe, by design.
...
///
/// Following the `rpt` crate's convention, a variant that carries a
/// [`source`](std::error::Error::source) does not interpolate it — render the whole chain with
/// [`rpt::error_chain`].
#[derive(Debug, thiserror::Error)]
#[non_exhaustive]
pub enum Error {
    /// An error from the `rpt` reader (opening or decoding the report).
    #[error(transparent)]
