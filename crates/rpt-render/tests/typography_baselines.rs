...
//! formulas), each isolating one font/text axis: face, size, style flags, color + alignment, and
...
//! the baseline that pins layout behaviour.
//!
...
//! ```
//!
//! **These baselines freeze this renderer's own behaviour, not the format's full semantics**: text
//! rotation is not applied, justified alignment is not stretched, `LineSpacing` is unmodelled, and
//! font size/`CharacterSpacing` are per-paragraph rather than per-run. A baseline changing when one
//! of those is implemented is the expected, correct signal — re-bless it then.
//!
...
//!
//! The trade-off is that `font_faces` exercises the *fallback* path rather than face variety:
//! [`CosmicLayout`] routes any family that is not in the DB through the sans-serif generic, so with
//! only the bundled faces loaded, `Times New Roman` and `Courier New` resolve to Liberation Sans
//! instead of the metric-compatible Liberation Serif/Mono. A fix to that mapping would move these
//! baselines, and that diff is the point.
...
use rpt_render::{CosmicLayout, DateTimeSpecials, FontProvider, Locale};
...
/// baseline depend on the wall clock. (The typography fixtures are static text; this keeps a fixture
/// that adds a date special honest.)
...
/// The stems of the committed Page-IR baselines in `dir`, sorted, so they can be compared against the
/// discovered fixture names.
fn baseline_names(dir: &Path) -> Vec<String> {
    let Ok(entries) = std::fs::read_dir(dir) else {
        return Vec::new();
    };
...
/// The document-scoped half of the Page IR, as a leading `// document` block. Pages alone would
/// leave [`PagedDocument::sections`] — produced by this very layer — invisible to its own baseline.
...
fn typography_render_matches_baselines() {
    ...
    // Fixtures and baselines are discovered, not listed, so either directory moving or emptying would
    // leave the loop below with nothing to compare and the harness green. Requiring the two sets to
    // agree is the floor: it catches a fixture that vanished, a baseline that was never blessed, and
    // a wrong directory, all as a failure rather than as a silent no-op.
    let blessed = baseline_names(&ir_dir);
    let named: Vec<String> = fixtures.iter().map(|(name, _)| name.clone()).collect();
    assert!(
        !fixtures.is_empty(),
        "no typography fixtures at {REPORTS}: nothing would be compared"
    );
    if !bless {
        assert_eq!(
            named, blessed,
            "the {} fixture(s) at {REPORTS} and the {} committed baseline(s) name different reports",
            named.len(),
            blessed.len()
        );
    }
    ...
    for (name, path) in &fixtures {
        let rpt = rpt_reader::Rpt::open(path).expect("open typography fixture");
        let report = rpt.report();

        // Render once, through the hermetic font stack, and serialize the Page IR from that one
        // document.
        ...
        // alone — a zero-page render would make the baseline vacuously "match".
        assert!(
            !doc.pages.is_empty(),
