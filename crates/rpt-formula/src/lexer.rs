...
fn classify_reference(inner: &str) -> (RefKind, String) {
    let (kind, name) = crate::token::split_reference(inner);
    (kind, name.to_string())
}

/// Scan a `#...#` date literal on a single line. Returns `None` when no closing `#` is found
/// before the next newline / EOF (caller emits a stray `#`). The internals are not parsed here;
/// a date body carries no braces/parens/reference sigils (only a textual `Month d, yyyy` comma),
/// so scanning to the closing `#` never hides a reference.
