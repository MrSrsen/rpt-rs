...
impl Parser {
    ...
    fn parse_range(&mut self) -> Node {
        // The unary range operators are prefixes at this level, not at the general prefix level:
        // `UpFrom 5 * 2` is a range over the product, matching the binary forms' operand grammar.
        if let Some(code) = self.word_op(&[
            ("upfrom", op::RANGE_UP_FROM),
            ("upfrombutnotincluding", op::RANGE_UP_FROM_EXCL),
            ("upto", op::RANGE_UP_TO),
            ("uptobutnotincluding", op::RANGE_UP_TO_EXCL),
        ]) {
            let op_span = self.cur().span();
            self.advance();
            let expr = self.parse_additive();
            return unary(code, expr, op_span);
        }
