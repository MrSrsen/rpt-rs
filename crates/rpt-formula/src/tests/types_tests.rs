...
use crate::types::{deduce_type, is_builtin, string_max_bytes, ResultKind as K};
...
fn builtin_lookup_is_case_insensitive() {
    assert_eq!(is_builtin("ToText"), is_builtin("totext"));
    assert!(is_builtin("Sum"));
    assert!(!is_builtin("definitely_not_a_builtin_xyz"));
}
