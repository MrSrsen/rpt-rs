...
fn repeated_name_is_located_per_occurrence() {
    // A name that appears more than once must be flagged at *each* of its own spans, not merely
    // once for the name. Both an unknown function and an unknown reference are exercised.
