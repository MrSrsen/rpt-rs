...
//!
//! The per-builtin lookup table in `types_table.rs` is `include!`d at the bottom of this module so it
//! shares this module's `ReturnRule`/`StrLenRule`/`Sig`. Downstream crates map a scalar
//! [`ResultKind`] to their own field-value-type enum.
...
/// A formula node's **result kind**. `Unknown` (0) covers statically-undeterminable nodes (an
/// undeclared variable, a runtime builtin, an unresolved name), which stay unresolved until
/// runtime/binding. The raw codes are laid out in blocks — scalars, then ranges, then arrays, then
/// range-arrays — so [`to_scalar`](Self::to_scalar) / [`to_array`](Self::to_array) /
/// [`to_range`](Self::to_range) are arithmetic on the code rather than a lookup.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
#[repr(u8)]
pub enum ResultKind {
    /// Statically undeterminable (undeclared variable, runtime builtin, unresolved name).
    Unknown = 0,
    // Scalars 1..7
    /// Number scalar.
    Number = 1,
    ...
    /// String scalar.
    String = 7,
    /// Bare callee leaf (a function reference).
    FunctionRef = 8,
    ...
    /// Array of Currency range.
    CurrencyRangeArray = 0x17,
    /// Array of Date range.
    DateRangeArray = 0x18,
    /// Array of Time range.
    TimeRangeArray = 0x19,
    /// Array of DateTime range.
    DateTimeRangeArray = 0x1a,
    /// Array of String range.
...
impl ResultKind {
    ...
    /// A numeric scalar (`Number` or `Currency`).
    pub fn is_numeric(self) -> bool {
        matches!(self, ResultKind::Number | ResultKind::Currency)
    }
    /// A temporal scalar (`Date`, `Time`, or `DateTime`).
    pub fn is_temporal(self) -> bool {
        matches!(
            self,
            ResultKind::Date | ResultKind::Time | ResultKind::DateTime
        )
    }
...
/// A builtin's **argument-count signature** — the shape its argument count must satisfy, carried per
/// name in the type table. Checked in one place per evaluator: [`crate::validate`] (an editor diagnostic)
...
/// Look up a builtin/function/special-field name (case-insensitive) → its typing rules.
pub(super) fn rules(name: &str) -> Option<Rules> {
    let lname = name.to_ascii_lowercase();
    BUILTINS
        .binary_search_by(|(n, ..)| n.cmp(&lname.as_str()))
        .ok()
        .map(|i| {
            let (_, ret, len, sig) = BUILTINS[i];
            (ret, len, sig)
        })
}

/// Whether `name` (case-insensitive) is a builtin, function or special field this engine knows.
...
pub fn deduce_type(
    ...
    match &node.kind {
        ...
        NodeKind::Ident(name) => match rules(name) {
            Some((ret, ..)) => apply_return_rule(ret, &[], ref_lookup),
            None => K::Unknown,
        },
        NodeKind::Call { name, args } => match rules(name) {
            Some((ret, ..)) => apply_return_rule(ret, args, ref_lookup),
        ...
        NodeKind::Unary { op, expr } => match *op {
            ...
            op::NOT => K::Boolean,
            // A half-open range is a range, not its bound's scalar kind — falling through to the
            // operand would type `UpFrom 10` as Number. `Unknown` matches how the binary `To`
            // operator is typed (it reaches `deduce_binary`'s catch-all), so both arities are
            // untyped alike rather than only one of them.
            c if op::is_unary_range(c) => K::Unknown,
...
/// Map a declared [`VarKind`](super::ast::VarKind) to its scalar [`ResultKind`].
fn var_kind_result(kind: super::ast::VarKind) -> ResultKind {
    use super::ast::VarKind as V;
    match kind {
        V::Number => ResultKind::Number,
        V::Currency => ResultKind::Currency,
        V::Boolean => ResultKind::Boolean,
        V::Date => ResultKind::Date,
        V::Time => ResultKind::Time,
        V::DateTime => ResultKind::DateTime,
        V::String => ResultKind::String,
    }
}
...
fn deduce_binary(
    ...
    match code {
        ...
        op::STAR | op::SLASH | op::BACKSLASH | op::MOD => numeric_promote(lt(), rt()),
        op::PERCENT => K::Number, // `%`
...
fn add_type(l: ResultKind, r: ResultKind) -> ResultKind {
    ...
    if l == K::String && r == K::String {
        ...
        numeric_promote(l, r)
    } else if l.is_temporal() && r.is_numeric() {
        l
    } else if r.is_temporal() && l.is_numeric() {
        r
...
fn string_max_bytes_raw(
    ...
    match &node.kind {
        ...
        NodeKind::Call { name, args } => {
            let Some((_, len_rule, _)) = rules(name) else {
                return MAX_STRING_BYTES;
            };
            call_str_bytes(len_rule, args, ref_type, ref_bytes)
        }
        NodeKind::Seq(stmts) => stmts
            .last()
            .map(|n| string_max_bytes_raw(n, ref_type, ref_bytes))
...
// The builtin type table. Included into this module so it shares `ReturnRule`/`StrLenRule`/`Sig`.
