...
pub trait EvalContext {
    ...
    /// Resolve a `GroupName({condition field})` reference to the named group's key — the same value a
    /// placed Group Name field prints. Like a summary function, the operand is a name rather than a
    /// value: `field` is the group's condition field (`table.field`, or `@formula` with its sigil
    /// restored), and the group it names need not be the innermost one.
    ///
    /// Returns `None` when this context has no grouping facility at all (the caller then reports the
    /// reference as unsupported); a facility that finds no such group returns `Some(Value::Null)`.
