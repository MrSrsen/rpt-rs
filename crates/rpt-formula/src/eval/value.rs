...
pub enum Value {
    ...
    ///
    /// Either bound may be [`Value::Null`], which means that end is **open** (unbounded) — the
    /// encoding of a half-open range parameter, whose open end stores no value at all (the SDK's
    /// `RangeBoundType::NoBound`). Membership ignores an open end rather than comparing against it.
    Range {
        /// The low bound, or [`Value::Null`] for an open low end.
        lo: Box<Value>,
        /// The high bound, or [`Value::Null`] for an open high end.
        hi: Box<Value>,
        /// Whether the low bound is included (irrelevant when it is open).
        lo_incl: bool,
        /// Whether the high bound is included (irrelevant when it is open).
        hi_incl: bool,
    },
