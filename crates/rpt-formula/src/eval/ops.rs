...
pub(super) fn apply_unary(code: u8, v: Value) -> Result<Value, EvalError> {
    // A range operator is excluded from the null gate for the same reason the binary ones are: a null
    // operand is an OPEN END, not an absent value, so `UpFrom Null` is a range with no lower limit
    // rather than `Null`. That is the rule the stored `RangeBoundType::NoBound` already encodes.
    if v.is_null() && !op::is_unary_range(code) {
        return Ok(Value::Null);
    }
    match code {
        // The half-open ranges: the operand is one bound and the other end is left unbounded.
        c if op::is_unary_range(c) => {
            let up = c == op::RANGE_UP_FROM || c == op::RANGE_UP_FROM_EXCL;
            let incl = c == op::RANGE_UP_FROM || c == op::RANGE_UP_TO;
            let (lo, hi) = if up {
                (v, Value::Null)
            } else {
                (Value::Null, v)
...
///
/// A range operand makes `=` a **membership** test rather than a comparison. That is not a
/// convenience: it is the shape the designer generates for a range parameter, `{Field} = {?Param}`,
/// so treating it as a type mismatch fails the record-selection formula on every row. Selection fails
/// open, so the report then renders with no detail rows at all — an empty document is the symptom, and
/// nothing in it says why.
fn values_eq(l: &Value, r: &Value) -> Result<bool, EvalError> {
    match (l, r) {
        (Value::Bool(a), Value::Bool(b)) => Ok(a == b),
        // Either side may hold the range: `=` is symmetric, and a formula is free to write the
        // parameter first.
        (_, Value::Range { .. }) => Ok(matches!(value_in(l, r)?, Value::Bool(true))),
        (Value::Range { .. }, _) => Ok(matches!(value_in(r, l)?, Value::Bool(true))),
...
fn value_in(l: &Value, r: &Value) -> Result<Value, EvalError> {
    match r {
        ...
        Value::Range {
            ...
            // A null value belongs to no range, however open: the engine's null comparisons never
            // hold.
            if l.is_null() {
                return Ok(Value::Bool(false));
            }
            let lo_ok = bound_holds(l, lo, *lo_incl, std::cmp::Ordering::Greater)?;
            let hi_ok = bound_holds(l, hi, *hi_incl, std::cmp::Ordering::Less)?;
...
/// Whether `v` satisfies one end of a range. `inside` is the ordering of `v` against the bound that
/// lies strictly within the range (`Greater` for the low end, `Less` for the high end), and `incl`
/// decides the `Equal` case.
///
/// A [`Value::Null`] bound is an **open** end (the SDK's `RangeBoundType::NoBound`, and what the `To`
/// operator builds from a null operand): it constrains nothing, so that side always holds. Comparing
/// against it instead would be a type mismatch, failing the whole record-selection formula.
fn bound_holds(
    v: &Value,
    bound: &Value,
    incl: bool,
    inside: std::cmp::Ordering,
) -> Result<bool, EvalError> {
    if bound.is_null() {
        return Ok(true);
    }
    let ord = compare(v, bound)?;
    Ok(if ord == std::cmp::Ordering::Equal {
        incl
    } else {
        ord == inside
    })
