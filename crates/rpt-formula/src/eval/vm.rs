...
enum Op {
    ...
    /// Resolve a `GroupName({condition field})` reference against the context's group state and push
    /// the named group's key. Carries the condition field — the operand is a name, not a stack value.
    GroupName(String),
...
impl Compiler {
    ...
    fn call(&mut self, name: &str, args: &[Node], span: Span) {
        ...
        // A group-name reference names its group by condition field, so it too takes no stack args.
        if let Some(gc) = GroupNameCall::from_call(name, args) {
            self.cur_span = span;
            self.emit(Op::GroupName(gc.field));
            return;
        }
...
fn exec_op(
    ...
    match &chunk.ops[ip] {
        ...
        Op::GroupName(field) => {
            let v = ctx
                .group_name(field)
                .ok_or_else(|| group_name_needs_context(field))?;
            stack.push(v);
        }
        Op::Bin(code) => {
