...
//! type table. Print-state specials route through [`EvalContext::special`]. Unless noted, a `Null`
...
enum Kind {
    ...
    /// format / calendar …) that evaluates to the stored ordinal of the enumeration it names —
    /// `rpt_model`'s decoded `Alignment`/`LineStyle`/`YearFormat`/`NegativeFormat`/… use the same
    /// numbering — so a conditional-format formula assigning it behaves like picking that option
    /// statically. Line-thickness constants carry twips instead (`crOnePointLine` = 20), matching
    /// what that property stores. A constant whose enumeration isn't identified is left out of
    /// [`TABLE`] rather than given a guessed number.
    Const(f64),
    /// A `cr*` constant naming an argument value of the date builtins rather than a stored format
    /// code: `firstDayOfWeek` (`crUseSystem`=0, `crSunday`=1 … `crSaturday`=7) and `firstWeekOfYear`
    /// (`crUseSystem`=0, `crFirstJan1`=1, `crFirstFourDays`=2, `crFirstFullWeek`=3). These are the
    /// numbers the builtins accept, so a formula may spell the argument either way.
    DateArg(f64),
...
const TABLE: &[(&str, Kind)] = &[
    ...
    ("crampmafter", Kind::Const(1.0)),
    ("crampmbefore", Kind::Const(0.0)),
    ("craqua", Kind::Color(16776960.0)),
    ("crascendingorder", Kind::Const(0.0)),
    ("crblack", Kind::Color(0.0)),
    ("crblue", Kind::Color(16711680.0)),
    ("crbold", Kind::Const(1.0)),
    ("crbolditalic", Kind::Const(3.0)),
    ("crbottomaligned", Kind::Const(8.0)),
    ("crbracketednegatives", Kind::Const(3.0)),
    ("crcenteredhorizontally", Kind::Const(2.0)),
    ("crcenteredvertically", Kind::Const(7.0)),
    ("crcustom", Kind::Const(2.0)),
    ("crcyan", Kind::Color(16776960.0)),
    ("crdashedline", Kind::Const(3.0)),
    ("crdateonly", Kind::Const(2.0)),
    ("crdatethentime", Kind::Const(0.0)),
    ("crdaymonthyear", Kind::Const(1.0)),
    ("crdayofweekinfwparentheses", Kind::Const(2.0)),
    ("crdayofweekinfwsquarebrackets", Kind::Const(4.0)),
    ("crdayofweekinparentheses", Kind::Const(1.0)),
    ("crdayofweekinsquarebrackets", Kind::Const(3.0)),
    ("crdayofweeknotenclosed", Kind::Const(0.0)),
    ("crdefaulthoraligned", Kind::Const(0.0)),
    ("crdefaultorientation", Kind::Const(0.0)),
    ("crdefaultveraligned", Kind::Const(0.0)),
    ("crdescendingorder", Kind::Const(1.0)),
    ("crdottedline", Kind::Const(4.0)),
    ("crdoubleline", Kind::Const(2.0)),
    ("crfirstfourdays", Kind::DateArg(2.0)),
    ("crfirstfullweek", Kind::DateArg(3.0)),
    ("crfirstjan1", Kind::DateArg(1.0)),
    ("crfixedcurrencysymbol", Kind::Const(1.0)),
    ("crfloatingcurrencysymbol", Kind::Const(2.0)),
    ("crfriday", Kind::DateArg(6.0)),
    ("crfuchsia", Kind::Color(16711935.0)),
    ("crgray", Kind::Color(8421504.0)),
    ("crgreen", Kind::Color(32768.0)),
    ("crgregoriancalendar", Kind::Const(1.0)),
    ("crgregorianenglishcalendar", Kind::Const(2.0)),
    ("crhairline", Kind::Const(0.0)),
    ("crhalfpointline", Kind::Const(10.0)),
    ("crhtmltext", Kind::Const(2.0)),
    ("critalic", Kind::Const(2.0)),
    ("crjapanesecalendar", Kind::Const(3.0)),
    ("crjustified", Kind::Const(4.0)),
    ("crlandscape", Kind::Const(2.0)),
    ("crleadingcurrencyinsidenegative", Kind::Const(0.0)),
    ("crleadingcurrencyoutsidenegative", Kind::Const(1.0)),
    ("crleadingdayofweek", Kind::Const(0.0)),
    ("crleadingminus", Kind::Const(1.0)),
    ("crleadingzeroday", Kind::Const(1.0)),
    ("crleadingzerohour", Kind::Const(0.0)),
    ("crleadingzerominute", Kind::Const(0.0)),
    ("crleadingzeromonth", Kind::Const(1.0)),
    ("crleadingzerosecond", Kind::Const(0.0)),
    ("crleftaligned", Kind::Const(1.0)),
    ("crlefttorighttextreadingorder", Kind::Const(0.0)),
    ("crlime", Kind::Color(65280.0)),
    ("crlongdayofweek", Kind::Const(1.0)),
    ("crlongera", Kind::Const(1.0)),
    ("crlongmonth", Kind::Const(3.0)),
    ("crlongyear", Kind::Const(1.0)),
    ("crmagenta", Kind::Color(16711935.0)),
    ("crmaroon", Kind::Color(128.0)),
    ("crmonday", Kind::DateArg(2.0)),
    ("crmonthdayyear", Kind::Const(2.0)),
    ("crnavy", Kind::Color(8388608.0)),
    ("crnocolor", Kind::Color(-1.0)),
    ("crnocurrencysymbol", Kind::Const(0.0)),
    ("crnoday", Kind::Const(2.0)),
    ("crnodayofweek", Kind::Const(2.0)),
    ("crnoera", Kind::Const(2.0)),
    ("crnohour", Kind::Const(2.0)),
    ("crnoline", Kind::Const(0.0)),
    ("crnominute", Kind::Const(2.0)),
    ("crnomonth", Kind::Const(4.0)),
    ("crnonegativesign", Kind::Const(0.0)),
    ("crnosecond", Kind::Const(2.0)),
    ("crnoyear", Kind::Const(2.0)),
    ("crnumericday", Kind::Const(0.0)),
    ("crnumerichour", Kind::Const(1.0)),
    ("crnumericminute", Kind::Const(1.0)),
    ("crnumericmonth", Kind::Const(0.0)),
    ("crnumericsecond", Kind::Const(1.0)),
    ("crolive", Kind::Color(32896.0)),
    ("croneandhalfpointline", Kind::Const(30.0)),
    ("croneorzero", Kind::Const(4.0)),
    ("cronepointline", Kind::Const(20.0)),
    ("croriginalorder", Kind::Const(2.0)),
    func("crpi", Builtin::Math(MathFn::Pi)),
    ("crportrait", Kind::Const(1.0)),
    ("crpurple", Kind::Color(8388736.0)),
    ("crred", Kind::Color(255.0)),
    ("crregular", Kind::Const(0.0)),
    ("crrightaligned", Kind::Const(3.0)),
    ("crrighttolefttextreadingorder", Kind::Const(1.0)),
    ("crrtftext", Kind::Const(1.0)),
    ("crsaturday", Kind::DateArg(7.0)),
    ("crshortdayofweek", Kind::Const(0.0)),
    ("crshortera", Kind::Const(0.0)),
    ("crshortmonth", Kind::Const(2.0)),
    ("crshortyear", Kind::Const(0.0)),
    ("crsilver", Kind::Color(12632256.0)),
    ("crsingleline", Kind::Const(1.0)),
    ("crsunday", Kind::DateArg(1.0)),
    ("crteal", Kind::Color(8421376.0)),
    ("crthreeandhalfpointline", Kind::Const(70.0)),
    ("crthreepointline", Kind::Const(60.0)),
    ("crthursday", Kind::DateArg(5.0)),
    ("crtimeonly", Kind::Const(3.0)),
    ("crtimethendate", Kind::Const(1.0)),
    ("crtopaligned", Kind::Const(6.0)),
    ("crtorf", Kind::Const(1.0)),
    ("crtrailingcurrencyinsidenegative", Kind::Const(2.0)),
    ("crtrailingcurrencyoutsidenegative", Kind::Const(3.0)),
    ("crtrailingdayofweek", Kind::Const(1.0)),
    ("crtrailingminus", Kind::Const(2.0)),
    ("crtrueorfalse", Kind::Const(0.0)),
    ("crtuesday", Kind::DateArg(3.0)),
    ("crtwelvehour", Kind::Const(0.0)),
    ("crtwentyfourhour", Kind::Const(1.0)),
    ("crtwoandhalfpointline", Kind::Const(50.0)),
    ("crtwopointline", Kind::Const(40.0)),
    ("cruninterpretedtext", Kind::Const(0.0)),
    ("crusesystem", Kind::DateArg(0.0)),
    ("crwednesday", Kind::DateArg(4.0)),
    ("crwhite", Kind::Color(16777215.0)),
    ("crwindowslong", Kind::Const(0.0)),
    ("crwindowsshort", Kind::Const(1.0)),
    ("cryearmonthday", Kind::Const(0.0)),
    ("cryellow", Kind::Color(65535.0)),
    ("cryesorno", Kind::Const(2.0)),
    ("cryorn", Kind::Const(3.0)),
    ...
    // `Today` is the engine's alias for `CurrentDate`: a data-time special the host supplies via
    // `EvalContext::special`, resolved before the print pass.
    ("today", Kind::Special),
...
/// [`EvalError::Unsupported`] when it is a known-but-unimplemented builtin (in the type table),
...
pub(super) fn resolve(
    ...
    match dispatch(&lname, args, ctx) {
        Some(r) => r,
        None => Err(if crate::types::is_builtin(&lname) {
...
pub(super) fn dispatch(
    ...
    Some(match lookup(name)? {
        ...
        Kind::Const(n) | Kind::DateArg(n) => Ok(Value::Number(n)),
        ...
        Kind::Func(builtin) => {
            ...
            if let Some((.., sig)) = crate::types::rules(name) {
...
mod tests {
    ...
    fn data_time_specials_are_not_print_state() {
        ...
        for name in ["currentdate", "datadate", "currentdatetime", "printdate"] {
    ...
    /// type table (`types_table.rs`), which drives return-type / argument validation. The two tables
    /// are maintained separately, so this guards against a new builtin being wired into the
    /// evaluator while its type-system entry is forgotten (or vice versa).
    #[test]
    fn implemented_funcs_are_typed() {
        use std::collections::HashSet;
        // The date/time *constructors* are handled specially by the type system (as type
        // constructors rather than ordinary calls), so they are legitimately absent from the type
        // table. Every other implemented function must be present.
        const CONSTRUCTOR_EXCEPTIONS: &[&str] = &["date", "datetime", "time"];
        let typed: HashSet<&str> = crate::types::BUILTINS.iter().map(|(n, ..)| *n).collect();
        for (name, kind) in TABLE {
            if matches!(kind, Kind::Func(..)) && !CONSTRUCTOR_EXCEPTIONS.contains(name) {
                assert!(
                    ...
                    "builtin `{name}` is implemented in eval::builtins TABLE but is missing from the \
                     type table (types_table.rs) — its return type / arg validation would be \
                     unknown (add it there, or to CONSTRUCTOR_EXCEPTIONS if it is a special)"
                );
            }
        }
    }
    ...
    /// functions and color constants — also produce a value, so `deduce_type` needs a type-table row
    /// to infer each one's result kind (an absent name deduces as `Unknown`). The names in
    /// `KNOWN_ABSENT` below are deliberately untyped rather than merely overlooked:
    /// `crcyan`/`crmagenta`/`rowcounter`/`timestring` are names this crate accepts as a superset of
    /// the language, and `next`/`nextvalue`/`previous`/`previousvalue` are prefix operators rather
    /// than calls. They still evaluate correctly (`dispatch` handles them directly); only their
    /// deduced type is `Unknown`. Typing the record-nav operators would need arg-type propagation,
    /// not a table row. This test locks the invariant from both sides: no *other* non-`Func` name may
    /// silently fall out of the type table, and any `KNOWN_ABSENT` name that ever gains a row must be
    /// removed from the list.
    #[test]
    fn non_func_names_typing_is_pinned() {
        use std::collections::HashSet;
        // Deliberately untyped: a superset name, or a prefix operator rather than a call.
        ...
        for (name, kind) in TABLE {
            if matches!(kind, Kind::Func(..)) {
                continue; // covered by `implemented_funcs_are_typed`
            }
            let present = typed.contains(name);
            if KNOWN_ABSENT.contains(name) {
                assert!(
                    !present,
                    "`{name}` is now in the type table — remove it from KNOWN_ABSENT"
                );
            } else {
                assert!(
                    present,
                    "non-Func name `{name}` drifted out of the type table (its deduced type would \
                     be Unknown); add a row, or add it to KNOWN_ABSENT if its typing is unknown"
                );
            }
        }
    }
    ...
    /// type table — a `Sig::Any` would let a wrong-arity call slip past the
    /// [`dispatch`](super::dispatch) arity gate. The date/time *constructors* have no row (see
    /// `implemented_funcs_are_typed`), so there is nothing to hang a signature on; they are excluded,
    /// matching that test's `CONSTRUCTOR_EXCEPTIONS`.
    #[test]
    fn dispatched_funcs_have_a_bounded_signature() {
        ...
        for (name, kind) in TABLE {
            ...
            let (.., sig) = crate::types::rules(name)
                .unwrap_or_else(|| panic!("dispatched builtin `{name}` is not in the type table"));
            assert!(
                !matches!(sig, crate::types::Sig::Any),
                "dispatched builtin `{name}` has an unbounded Sig::Any signature — give it a real \
                 argument-count bound in types_table.rs"
            );
        }
    }

    /// Every typed `cr*` constant must be dispatchable — a color ([`Kind::Color`]), a date-builtin
    /// argument ([`Kind::DateArg`]), the `crpi` alias, or a bare [`Kind::Const`] — so no
    /// conditional-format formula returning a `cr*` constant falls through to
    /// [`EvalError::Unsupported`]. The exceptions are the constants whose enumeration is not
    /// identified: a `Kind::Const` for one would have to invent a number, and reporting the name as
    /// unimplemented is the honest answer. Identifying the enumeration removes it from this list.
    #[test]
    fn all_cr_constants_are_dispatchable() {
        const VALUE_NOT_ESTABLISHED: &[&str] =
            &["crlongleadingday", "crnoleadingday", "crshortleadingday"];
        for (name, ..) in crate::types::BUILTINS {
            if !name.starts_with("cr") {
                continue;
            }
            if VALUE_NOT_ESTABLISHED.contains(name) {
                assert!(
                    super::lookup(name).is_none(),
                    "`{name}` now dispatches — remove it from VALUE_NOT_ESTABLISHED"
                );
                continue;
            }
            assert!(
                super::lookup(name).is_some(),
                "cr* constant `{name}` is in the type table but not dispatched (Unsupported)"
            );
        }
    }

    /// Each bare `cr*` [`Kind::Const`] evaluates to the ordinal its enumeration stores, so it is a
    /// small number. This guards the whole family against a regression to per-name codes drawn from
    /// some unrelated numbering, which would be wrong everywhere at once and silently so.
    #[test]
    fn const_values_are_enum_ordinals() {
        for (name, kind) in TABLE {
            if let Kind::Const(v) = kind {
                assert!(
                    *v >= 0.0 && *v <= 70.0 && v.fract() == 0.0,
                    "`{name}` = {v} is not an enumeration ordinal (or a thickness in twips)"
                );
            }
        }
    }

    /// One value per enumeration, pinned against `rpt_model`'s decoded ordinals for the same
    /// property. A formula assigning one of these to a conditional format must land on the option
    /// the static property would.
    #[test]
    fn const_values_match_the_stored_enumerations() {
        const PINNED: &[(&str, f64)] = &[
            ...
            ("crsingleline", 1.0),
            ("crdottedline", 4.0),
            ("cronepointline", 20.0), // thickness is twips, not an ordinal
            ("crlongyear", 1.0),
            ("crlongmonth", 3.0),
            ("crhtmltext", 2.0),
            ("croneorzero", 4.0),
            ("crbracketednegatives", 3.0),
            ("crfloatingcurrencysymbol", 2.0),
            ("crlandscape", 2.0),
            ("crdescendingorder", 1.0),
        ];
        for (name, want) in PINNED {
            assert!(
                matches!(super::lookup(name), Some(Kind::Const(v)) if v == *want),
                "`{name}` should evaluate to {want}"
            );
        }
    }

    /// The date-builtin argument constants carry the number the argument takes, not an opaque code.
    /// The two families share `crUseSystem`=0; the rest number the seven weekdays from Sunday and
    /// the three week-of-year rules from `crFirstJan1`.
    #[test]
    fn date_arg_constants_are_argument_values() {
        ...
        for (name, want) in expected {
            match super::lookup(name) {
                Some(Kind::DateArg(v)) => assert_eq!(v, *want, "`{name}` argument value"),
                other => panic!("`{name}` is not a DateArg: {:?}", other.is_some()),
            }
        }
        let tabled = TABLE
            .iter()
            .filter(|(_, k)| matches!(k, Kind::DateArg(_)))
            .count();
        assert_eq!(tabled, expected.len(), "an untested DateArg entry exists");
    }

    /// Every relative-date-range constant dispatches as a [`Kind::DateRange`], so a record-selection
    /// formula using one evaluates to a range rather than failing as unsupported. The set is taken
    /// from the type table — a name typed as returning a date range — so the two tables cannot drift
    /// into disagreeing about which names those are.
    #[test]
    fn all_date_range_constants_are_dispatchable() {
        use std::collections::HashMap;
        let tabled: HashMap<&str, &Kind> = TABLE.iter().map(|(n, k)| (*n, k)).collect();
        let mut count = 0;
        for (name, ret, ..) in crate::types::BUILTINS {
            if !matches!(ret, crate::types::ReturnRule::Fixed(k)
                if *k == crate::types::ResultKind::DateRange.raw())
            {
                continue;
            }
            assert!(
                matches!(tabled.get(name), Some(Kind::DateRange(_))),
