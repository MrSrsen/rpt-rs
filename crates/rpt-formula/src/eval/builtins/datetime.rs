...
/// The optional `firstDayOfWeek` argument: `crSunday`=1 … `crSaturday`=7. `crUseSystem`=0 names the
/// host's default first day and an omitted argument the language default; both are taken as Sunday.
...
/// week (starting on `first`) that has at least `threshold` days in the new year. A `threshold` of 1
/// implements the `crFirstJan1` rule, 4 `crFirstFourDays` (ISO-8601-like), 7 `crFirstFullWeek`.
...
mod tests {
    ...
    fn datepart_first_four_days_is_iso8601() {
        // With a Monday start, the first-four-days rule (mode 2) is exactly ISO-8601; these are
    ...
    fn datepart_first_full_week() {
        // The full-week rule (mode 3), Sunday start: week 1 is the first full Sun–Sat week. 2004-01-01
    ...
    fn datepart_first_jan1_never_rolls_forward() {
        // The Jan-1 rule (mode 1) keeps late-December in the current year (VBA semantics), unlike the
        // ISO-style modes.
    ...
    fn firstdayofweek_threading() {
        // A Monday first day (2) counts Monday-boundary crossings.
        ...
        // WeekdayName's n is relative to firstDayOfWeek: (3, abbrev, Monday-start) → Wed.
        assert_eq!(text("WeekdayName(3, true, 2)"), "Wed");
        assert_eq!(text("WeekdayName(1, false, 2)"), "Monday");
        // A first day of 0 (the host default) behaves as Sunday.
        assert_eq!(num("Weekday(#1/3/2004#, 0)"), 7.0);
    }

    /// The `cr*` day-of-week and week-of-year names are the spelling a report author uses for these
    /// arguments, so each must evaluate to the number the argument takes.
    #[test]
    fn named_constants_are_accepted_as_arguments() {
        assert_eq!(
            num(r#"DateDiff("ww", #5/1/2003#, #6/1/2003#, crMonday)"#),
            num(r#"DateDiff("ww", #5/1/2003#, #6/1/2003#, 2)"#)
        );
        // 2004-01-01 was a Thursday: the 4th day of a Monday-first week, the 5th of a Sunday-first.
        assert_eq!(num("DayOfWeek(#1/1/2004#, crMonday)"), 4.0);
        assert_eq!(num("DayOfWeek(#1/1/2004#, crSunday)"), 5.0);
        assert_eq!(num("Weekday(#1/1/2004#, crUseSystem)"), 5.0);
        assert_eq!(text("WeekdayName(3, False, crMonday)"), "Wednesday");
        // A Monday start under the first-four-days rule is ISO-8601: 2007-12-31 is 2008-W01.
        assert_eq!(
            num(r#"DatePart("ww", #12/31/2007#, crMonday, crFirstFourDays)"#),
            1.0
        );
        assert_eq!(
            num(r#"DatePart("ww", #12/31/2007#, crMonday, crFirstJan1)"#),
            53.0
        );
        assert_eq!(
            num(r#"DatePart("ww", #1/1/2004#, crSunday, crFirstFullWeek)"#),
            52.0
        );
    }
}
