...
pub(super) enum DateRange {
    /// `weektodatefromsun`: the current (Sunday-started) week up to `today`.
    WeekToDateFromSun,
    /// `monthtodate`: the 1st of the current month up to `today`.
    MonthToDate,
    /// `yeartodate`: Jan 1 of the current year up to `today`.
    YearToDate,
    /// `last7days`: the seven days ending at `today` (`today − 6 .. today`).
    Last7Days,
    /// `last4weekstosun`: the four weeks (28 days) ending at the nearest past Sunday.
    Last4WeeksToSun,
    /// `lastfullweek`: the previous complete Sunday–Saturday week.
    LastFullWeek,
    /// `lastfullmonth`: the whole previous calendar month.
    LastFullMonth,
    /// `alldatestotoday`: every date up to and including `today`.
    AllDatesToToday,
    /// `alldatestoyesterday`: every date up to and including `today − 1`.
    AllDatesToYesterday,
    /// `alldatesfromtoday`: `today` and every later date.
    AllDatesFromToday,
    /// `alldatesfromtomorrow`: `today + 1` and every later date.
    AllDatesFromTomorrow,
    /// `aged0to30days`: `today − 30 .. today`.
    Aged0To30Days,
    /// `aged31to60days`: `today − 60 .. today − 31`.
    Aged31To60Days,
    /// `aged61to90days`: `today − 90 .. today − 61`.
    Aged61To90Days,
    /// `over90days`: every date older than 90 days (up to `today − 91`).
    Over90Days,
    /// `next30days`: `today .. today + 30`.
    Next30Days,
    /// `next31to60days`: `today + 31 .. today + 60`.
    Next31To60Days,
    /// `next61to90days`: `today + 61 .. today + 90`.
    Next61To90Days,
    /// `next91to365days`: `today + 91 .. today + 365`.
    Next91To365Days,
    /// `calendar1stqtr`: Jan 1 – Mar 31 of the current year.
    Calendar1stQtr,
    /// `calendar2ndqtr`: Apr 1 – Jun 30 of the current year.
    Calendar2ndQtr,
    /// `calendar3rdqtr`: Jul 1 – Sep 30 of the current year.
    Calendar3rdQtr,
    /// `calendar4thqtr`: Oct 1 – Dec 31 of the current year.
    Calendar4thQtr,
    /// `calendar1sthalf`: Jan 1 – Jun 30 of the current year.
    Calendar1stHalf,
    /// `calendar2ndhalf`: Jul 1 – Dec 31 of the current year.
    Calendar2ndHalf,
    /// `lastyearmtd`: last year's 1st-of-this-month up to `today` a year ago.
    LastYearMtd,
    /// `lastyearytd`: last year's Jan 1 up to `today` a year ago.
    LastYearYtd,
}
