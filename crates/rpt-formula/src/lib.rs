...
//! dependency-free leaf, for the `Date`/`Time` a [`eval::Value`] carries) and **not** on the
//! `rpt-reader` decoder. That keeps it reusable on its own — by a formula language server, a WASM
//! formula sandbox, or a standalone validator/playground — without pulling in the whole binary reader.
//! A consumer that also needs `rpt-model`'s types owns any [`types::ResultKind`]-to-model-type mapping
//! itself, so no such bridge couples this crate to that model.
//!
//! Layers:
...
//!   context) unknown references — the source for a formula language server.
//!
...
pub use types::{deduce_type, is_builtin, string_max_bytes, ResultKind};
