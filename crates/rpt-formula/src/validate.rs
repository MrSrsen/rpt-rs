...
//!    by the per-builtin signature in the type table. The dispatched built-ins and the
...
use super::types::{deduce_type, rules, ResultKind};
...
impl Walker<'_> {
    ...
    fn check_call(&mut self, name: &str, args: &[Node], span: Span) {
        ...
        match rules(name) {
            ...
            Some((.., sig)) => {
                if let Some(msg) = arity_error(name, sig, args.len()) {
...
/// accepted (or the builtin carries no bound).
fn arity_error(name: &str, sig: super::types::Sig, n: usize) -> Option<String> {
...
fn nearest_builtin(name: &str) -> Option<String> {
    ...
    for (cand, ..) in super::types::BUILTINS {
