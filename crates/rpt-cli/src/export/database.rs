...
//!
//! Turns the decoded [`rpt`] model (plus the derived analytics from the `analysis` module) into XML
//! elements. Derived values — field `UseCount`, parameter usage flags — are computed there and
...
use crate::export::util::{
    b, escape, escape_text, field_type_name, value_type_name, write_collection,
};
...
pub(crate) fn write_database(o: &mut String, report: &Report, use_counts: &HashMap<String, i32>) {
    ...
    // TableLinks.
    write_collection(o, "    ", "TableLinks", &db.links, |o, link| {
...
pub(crate) fn write_parameter(
    ...
    let _ = writeln!(
        o,
        "      <ParameterFieldDefinition AllowCustomCurrentValues=\"{accv}\" DefaultValueDisplayType=\"{disp:?}\" DefaultValueSortMethod=\"BasedOnValue\" DefaultValueSortOrder=\"{sort:?}\" DiscreteOrRangeKind=\"DiscreteValue\" EditMask=\"\" EnableAllowEditingDefaultValue=\"{eaedv}\" EnableAllowMultipleValue=\"{eamv}\" EnableNullValue=\"False\" FormulaName=\"{{?{n}}}\" HasCurrentValue=\"{hcv}\" IsOptionalPrompt=\"{opt}\" Kind=\"{kind}\" MaximumValue=\"\" MinimumValue=\"\" Name=\"{n}\" NumberOfBytes=\"{nb}\" ParameterFieldName=\"{n}\" ParameterFieldUsage=\"{usage}\" ParameterType=\"{pt:?}\" ParameterValueKind=\"{vk:?}\" PromptText=\"{prompt}\" ReportName=\"{rn}\" ValueType=\"{vt}\">",
        kind = FieldKind::ParameterField.name(),
        disp = p.default_value_display_type,
        sort = p.default_value_sort_order,
        n = escape(name),
...
fn write_value_list(
    ...
    write_collection(o, "        ", container, values, |o, v| {
        if with_description {
            let _ = writeln!(
                o,
