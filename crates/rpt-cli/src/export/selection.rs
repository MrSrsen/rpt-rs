...
//!
//! This deliberately keeps its own small lexer/parser instead of reusing `crystal-formula` (as
//! `rpt-query` does). The two solve different problems: `rpt-query` translates the AST to *SQL*, a
//! semantic lowering where the authored surface syntax is irrelevant; this re-serializes the engine's
//! canonical *Crystal text* **byte-for-byte** (the sha256 parity gate), which is surface-preserving.
//! `crystal-formula`'s parser decodes string literals — its `Node::Str` holds the quote-stripped,
//! escape-resolved value with no source span — so the AST can no longer reproduce the raw authored
//! literal that the `IN`-operand rule needs; driving the emitter would mean dropping back to the token
//! stream and rebuilding the grammar, i.e. this parser again, at real regression risk on a hard
//! byte-identity gate for no functional gain. The narrow boolean-only grammar is also a feature: it
//! cannot mis-handle a construct it does not model.
